...
export default function ChatBox({
  ...
  return selectedFriend ? (
          ...
          <div className="flex flex-col gap-y-1.5 h-[450px] overflow-y-auto w-full px-3">
            {messages.map((message, index) => (
              ...
              >
                <div
                  key={index}
                  className={`flex items-center w-fit max-w-[80%] p-3 text- shadow-md ${
                    message.senderId === currentUser._id
                      ? "bg-blue-300 rounded-l-md"
                      : "bg-indigo-300 rounded-r-md"
                  }`}
                >
                  {message.type === "file" ? (
                    <FileHandler url={message.content} />
                  ) : (
                    message.content
                  )}
                </div>
    ...
    <div className="flex items-center justify-center h-max w-full animate-pulse">
      <h4 className="pt-[30%] text-xl font-medium text-stone-700">
